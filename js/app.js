(() => {
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) {
            new swiper_core_Swiper(".advantages__slider", {
            ...
            new swiper_core_Swiper(".services__slider", {
                modules: [ Pagination, Autoplay ],
                observer: true,
                observeParents: true,
                slidesPerView: 2.3,
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        const blocks = document.querySelectorAll("[data-parallax-block]");
        blocks.forEach((block => {
            const teths = block.querySelectorAll(".teth");
            block.addEventListener("mousemove", (function(e) {
                const {left, top, width, height} = block.getBoundingClientRect();
                const x = e.clientX - left - width / 2;
                const y = e.clientY - top - height / 2;
                teths.forEach((teth => {
                    const depth = parseFloat(teth.dataset.depth) || 10;
                    const moveX = (x / depth).toFixed(2) + "px";
                    const moveY = (y / depth).toFixed(2) + "px";
                    teth.style.setProperty("--x", moveX);
                    teth.style.setProperty("--y", moveY);
                }));
            }));
            block.addEventListener("mouseleave", (function() {
                teths.forEach((teth => {
                    teth.style.setProperty("--x", "0px");
                    teth.style.setProperty("--y", "0px");
                }));
            }));
        }));
    }));
    document.addEventListener("DOMContentLoaded", (() => {
        ...
        function stopAllExcept(activeIndex) {
            videos.forEach(((v, i) => {
                if (i !== activeIndex) {
                    v.pause();
                    v.currentTime = 0;
    ...
    document.querySelectorAll("form").forEach((form => {
        form.addEventListener("submit", (function(event) {
            ...
            [ "utm_source", "utm_medium", "utm_campaign", "utm_term", "utm_content" ].forEach((param => {
                ...
                if (input) input.value = urlParams.get(param) || "";
            }));
            const formData = new FormData(form);
            fetch("components/sendmail/webhoock.php", {
                method: "POST",
                body: formData
            }).then((async response => {
                ...
                if (data.status === "success") window.location.href = "thanks.html"; else alert("Помилка при відправці форми.");
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        function handlePhoneInput(event, phoneElement) {
            let value = phoneElement.value.replace(/\D/g, "");
            if (value.length > maxLength) value = value.slice(0, maxLength);
            phoneElement.value = formatPhoneNumber(value);
            if (event.type === "blur" && value.length !== maxLength) {
                phoneElement.value = "";
                phoneElement.classList.add("_form-error");
            } else phoneElement.classList.remove("_form-error");
        }
        phoneInputs.forEach((phoneElement => {
            if (!phoneElement.value || phoneElement.value.startsWith("+")) phoneElement.value = `+${countryCode} `;
            phoneElement.placeholder = "+38 (___) ___ __ __";
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        ...
        function processElements(selectors, onlyMobile = false) {
        ...
        processElements("p");
        processElements("h1, h2, h3, h4, h5, h6", true);
        processElements("span", true);
    }));
    window["FLS"] = true;
    menuInit();
    spollers();
